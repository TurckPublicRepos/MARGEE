...
| **VAR_RETAIN**   | Retain variables can be reached anywhere in the program. Values stored in this segment will be retained between power cycles. A `VAR_RETAIN` segment can only be declared at the top-level scope.<br/><br/>⚠️ Warning:<br/>Writing extensively to retained variables can shorten the device's lifespan because the device's flash memory supports a limited number of write cycles. |
